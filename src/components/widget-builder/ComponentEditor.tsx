...
const ComponentEditor: React.FC<ComponentEditorProps> = ({
          ...
          {component.apiConfig ? (
            ...
            <div>
              ...
              <Button 
                ...
                className="w-full"
              >
                <Plus size={14} className="mr-1" /> Create New API
              </Button>
            </div>
          )}
        </div>
  ...
  // The complete component structure with all the required closing tags
  return (
    <div className={`border border-gray-200 rounded-md mb-2 ${component.selected ? 'ring-2 ring-blue-500' : ''}`}>
      <div 
        className={`flex items-center justify-between p-3 cursor-pointer ${isExpanded ? 'border-b border-gray-200' : ''}`}
      ...
      {isExpanded && (
        <div className="p-3">
          <div className="mb-4">
            ...
            {selectedTooltip && (
              <div className="mt-2 p-2 bg-gray-50 border rounded-md text-xs">
                <p className="font-medium">{selectedTooltip.title}</p>
                <p className="mt-1 text-gray-600">{selectedTooltip.content}</p>
                {selectedTooltip.tags && selectedTooltip.tags.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {selectedTooltip.tags.map((tag, idx) => (
                      <Badge key={idx} variant="outline" className="px-1.5 py-0.5 text-xs">
                        {tag}
                      </Badge>
                    ))}
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end space-x-2">
              <Button
                ...
                className={shouldDisableRemove ? 'opacity-50 cursor-not-allowed' : ''}
              >
