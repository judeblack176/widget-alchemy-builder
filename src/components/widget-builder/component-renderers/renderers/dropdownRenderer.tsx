...
import { ChevronDown } from '../iconUtils';

export const dropdownRenderer = (finalProps: Record<string, any>) => {
  return (
      ...
      <div className={`relative border rounded overflow-hidden ${finalProps.searchable ? 'flex items-center' : ''}`}>
        ...
        >
          ...
          {Array.isArray(finalProps.options) ? 
            ...
            (finalProps.options || "Option 1,Option 2,Option 3").split(',').map((option: string, index: number) => (
